...
async def get_response(message):
    print(f"Sending to LLM: {message}")  # Add this line for logging
    global message_history
    print(f"Current message history: {message_history}")

    message_history.append({"role": "user", "content": "say this is a test"})
    print(f"New message history: {message_history}")
    chat_completion = client.chat.completions.create(
        messages=message_history,
        model="gpt-3.5-turbo",
    )
    response = chat_completion.choices[0].message.content
    print(f"Response from LLM: {response}")  # Add this line for logging
    return response
    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        for result in results:
            ...
            if not result.is_partial:
                ...
                # Await the asynchronous get_response function
                response = await get_response(self.current_transcript)
                print("Response:", response)
                audio_queue.put_nowait(response)
...
async def tts_from_queue():
    while True:
        if not audio_queue.empty():
            ...
            print(f"Synthesizing response: {text_chunk}")
            _, audio_file_path = synthesize_speech(text_chunk, enable_streaming=True)
            play_audio(audio_file_path, enable_streaming=True)
        await asyncio.sleep(0.1)  # Prevents the loop from being too aggressive
...
async def process_and_respond(text):
    ...
    async for chunk in stream:
        audio_queue.put_nowait(chunk.choices[0].message.content)
...
if __name__ == "__main__":
    loop = asyncio.get_event_loop()
    loop.create_task(tts_from_queue())
